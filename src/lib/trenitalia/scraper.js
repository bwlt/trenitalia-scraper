...
const scrapeFromTime = (fromDate: moment, $node: Selection): moment => {
  ...
  if (value.split(":").length !== 2)
    throw new ScrapeError(
      `Invalid scraped 'fromTime' format. Found '${value}'`
    );
...
const scrapeToTime = (fromDate: moment, $node: Selection): moment => {
  ...
  if (value.split(":").length !== 2)
    throw new ScrapeError(`Invalid scraped 'toTime' format. Found '${value}'`);
  const [h, m] = value.split(":").map(v => Number.parseInt(v));
...
const scrapeDuration = ($node: Selection): Duration => {
  ...
  if (!matches)
    throw new ScrapeError(`Invalid 'duration' format. Found '${valueStr}'`);
...
export function scrapeSolutions(htmlStr: string): SolutionObject[] {
  ...
  if ($solutionRow.length === 0)
    throw new ScrapeError(`'.solutionRow' HTML Elements not found`);
  ...
  $solutionRow.each((index, node) => {
    ...
    if ($tableCells.length !== 6)
      throw new ScrapeError(`Expected to find 6 columns in a '.solutionRow'`);
    ...
    if ($trainOffers.length === 0)
      throw new Error(`'.trainOffer' HTML Elements not found`);
    const trains: $PropertyType<SolutionObject, "trains"> = [];
    $trainOffers.each((index, node) => {
      const $node = $(node);
      const descrNode = $node.find(".descr").get(0);
      if (descrNode.type !== "tag")
        throw new ScrapeError(
          "Expecting a node type tag for scraping train offer"
        );
